...
Chrome profile sign-in alone is not enough for extension state to carry over. Chrome sync scopes extension data by extension ID, so both devices must run the same Resume extension ID. This repository pins the ID through the `manifest.json` `key` field, which keeps unpacked installs aligned as long as both devices load this same source tree.

To verify alignment, open `chrome://extensions` on each device with Developer mode enabled and compare the Resume extension ID. The IDs must be identical. In-app, the `Google Sync` panel now also prints the local extension ID and active storage backend so you can confirm runtime conditions from the new-tab UI.
...
Sync timing is eventual rather than instant. Resume writes with a short local debounce and then relies on Chrome Sync transport, so cross-device appearance usually lands within seconds but can sometimes take a few minutes.
