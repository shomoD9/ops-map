...
export async function loadDevicePrefs() {
    ...
    return new Promise((resolve) => {
      chrome.storage.local.get([DEVICE_PREFS_KEY], (result) => {
        if (chrome.runtime?.lastError) {
          console.warn("Ops Map: failed to read local device preferences.", getRuntimeErrorMessage());
  ...
  try {
    ...
    console.warn("Ops Map: failed to read local preference fallback storage.", error);
    return { ...DEFAULT_DEVICE_PREFS };
  }
}
...
export async function saveDevicePrefs(nextPrefs) {
  ...
  try {
    ...
    console.warn("Ops Map: failed to save local preference fallback storage.", error);
  }
}
...
export function subscribeToDevicePrefsChanges(onChange) {
  ...
  const listener = (event) => {
    ...
    try {
      ...
      console.warn("Ops Map: failed to parse local preference storage event payload.", error);
    }
  };
