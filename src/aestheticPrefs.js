/*
This file stores the selected visual aesthetic for Resume.
...
export async function loadAestheticPref() {
    ...
    return new Promise((resolve) => {
      chrome.storage.sync.get([AESTHETIC_STORAGE_KEY], (result) => {
        if (chrome.runtime?.lastError) {
          console.warn("Resume: failed to read aesthetic preference.", getRuntimeErrorMessage());
  ...
  try {
    ...
    console.warn("Resume: failed to read local aesthetic preference fallback.", error);
...
export async function saveAestheticPref(aesthetic) {
  ...
  try {
    ...
    console.warn("Resume: failed to save local aesthetic preference fallback.", error);
  }
}
